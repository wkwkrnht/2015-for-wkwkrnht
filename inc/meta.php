...
<meta name="keywords" content="<?php if(is_category()){echo get_meta_keyword_from_category();}elseif(is_single()){$posttags = get_the_tags();if($posttags){foreach($posttags as $tag){echo $tag->name . ','; }}the_title();}elseif(!empty($custom['meta_keywords'][0])){echo['meta_keywords'][0];}else{echo get_option('blogkeyword');}?>">
