...
<head>
	...
	<style amp-custom>
		...
		article{padding:20vh 3vw 5vh 3vw;box-shadow:0 1px 6px rgba(0,0,0,.12);}header{background-color:#ebeef7;text-align:center;margin:5px 0;}.bread{color:#ddd;}.bread .sp{color:#333;margin:0 .5em;}span .meta{padding:2px;}amp-img{max-width:100%;max-height:40vh;}
	...
	<article>
		...
		<section>
			<?php $pattern=array('/https:\/\/twitter.com\/.*\/status\/(.*).*/i',
				...
				add_shortcode('scshot',function($atts){extract(shortcode_atts(array('url'=>'',),$atts));return'<a href="' . $url . '" target="_blank">' . $url . '</a>';});
				add_shortcode('myqrcode',function($atts){extract(shortcode_atts(array('url'=>'','size'=>'',),$atts));return'<a href="' . $url . '" target="_blank">' . $url . '</a>';});
				add_shortcode('embedly',function($atts){extract(shortcode_atts(array('url'=>'',),$atts));return'<a href="' . $url . '" target="_blank">' . $url . '</a>';});
				add_shortcode('hatenaBlogcard',function($atts){extract(shortcode_atts(array('url'=>'',),$atts));return'<a href="' . $url . '" target="_blank">' . $url . '</a>';});?>
		</section>
		<footer id="flex">
			<div class="content">
