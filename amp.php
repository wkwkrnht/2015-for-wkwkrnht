...
<head>
	...
	<style amp-custom>
		...
		article{padding:20vh 3vw 5vh 3vw;box-shadow:0 1px 6px rgba(0,0,0,.12);}header{background-color:#ebeef7;text-align:center;margin:5px 0;}.bread{color:#ddd;}.bread .sp{color:#333;margin:0 .5em;}span .meta{padding:2px;}amp-img{max-width:50%;max-height:10%;}
	...
	<article>
		...
		<section>
			<?php $pattern=array('/https:\/\/twitter.com\/.*\/status\/(.*).*/i',
				...
				function amp_shortcode($atts){extract(shortcode_atts(array('url'=>'',),$atts));return'<a href="' . $url . '" target="_blank">' . $url . '</a>';}
				add_shortcode('scshot','amp_shortcode');
				add_shortcode('myqrcode','amp_shortcode');
				add_shortcode('embedly','amp_shortcode');
				add_shortcode('hatenaBlogcard','amp_shortcode');?>
		</section>
		<footer id="flex">
			<div class="content">
