...
<head>
	...
	<style amp-custom>
		...
		article{padding:20vh 3vw 5vh 3vw;box-shadow:0 1px 6px rgba(0,0,0,.12);}header{background-color:#ebeef7;text-align:center;margin:5px 0;}.bread{color:#ddd;}.bread .sp{color:#333;margin:0 .5em;}span .meta{padding:2px;}amp-img article{max-width:50%;max-height:10%;}
	...
	<article>
		...
		<section>
			<?php $pattern=array('/https:\/\/twitter.com\/.*\/status\/(.*).*/i',
				...
				'/<iframe/i','/<img/i','/\/>/','/\[scshot url="(.*?)"\]/','/\[myqrcode url="(.*?)"\]/','/\[embedly url="(.*?)"\]/','/\[hatenaBlogcard url="(.*?)"\]/');
